...
const Detail = ({ postDetails }: IProps) => {
  ...
  const handleLike = async (like: boolean) => {
    if (userProfile) {
      const { data, request } = await axios.put(`${BASE_URL}/api/post/like`, { userId: user?.id, postId: post._id, like });
  ...
  return (
    <div className='flex w-full absolute left-0 top-0 bg-white flex-wrap lg:flex-nowrap'>
      <div className='relative flex-2 w-[1000px] lg:w-9/12 flex justify-center items-center bg-black bg-no-repeat bg-cover bg-center'>
        <div className='absolute top-5 left-2 lg:left-6 flex gap-6 z-50 '>
        ...
        <div className='relative'>
          <div className='lg:h-[100vh] h-[60vh] flex items-center'>
            <video
              ...
              className="cursor-pointer lg:h-[85vh]"
            >
            </video>
          </div>
      ...
      <div className="relative w-[1000px] md:w-[900px] lg:w-[450px]">
          ...
          <div>
            <div className='flex gap-3 p-2 cursor-pointer font-semibold rounded'>
              <div className='ml-4 md:w-20 md:h-20 w-16 h-16'>
                <Link href="/">
                  <>
                    <Image width={62} height={62} className="rounded-full" src={post.postedBy.image} alt="r" />
                  </>
                </Link>
              </div>
              <Link href="/">
...
export const getServerSideProps = async ({ params: { id } }: Data) => {
